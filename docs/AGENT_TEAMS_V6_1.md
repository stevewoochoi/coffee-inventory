...
#### TASK-107 | 상단 메뉴 그룹화 + 역할별 라우트 분기

**배경**: V6에서 메뉴가 16개 이상으로 폭증. 1단 나열 → **6개 그룹 + 드롭다운**으로 재구조화.
...
**A. AdminLayout 메뉴 데이터 구조**:

```typescript
// types/navigation.ts
interface NavGroup {
  key: string;           // i18n key (nav.product, nav.ordering 등)
  icon: LucideIcon;      // 그룹 아이콘
  to?: string;           // 하위 없이 직접 이동 (대시보드)
  children?: NavItem[];  // 드롭다운 하위 메뉴
  roles: string[];       // 접근 가능 역할
}
...
KR_INVENTORY:
  [대시보드] [상품관리▾] [발주운영▾] [재고▾] [설정▾]
  → 재무 그룹 숨김, 설정에서 배송정책 숨김
...
JP_ORDERER:
  → StoreLayout 사용 (AdminLayout 아님)
  → 하단 탭: [대시보드] [발주] [주문현황] [입고] [재고]

FULFILLMENT:
  [대시보드] [수주 관리]
  → 발주운영 중 수주 관리만 단독 링크로 노출 (드롭다운 불필요)

STORE_MANAGER:
  → StoreLayout 사용 (기존 유지)
```

**C. NavDropdown 컴포넌트 구현**:

```typescript
// components/nav/NavDropdown.tsx
// 데스크톱: hover 시 드롭다운 펼침 (200ms delay로 실수 방지)
// 모바일: 클릭 시 아코디언 펼침
// 현재 하위 라우트 active 시 그룹 라벨도 active 스타일
// 드롭다운 항목에 아이콘 + 라벨
// 외부 클릭 시 닫힘
...
// 각 항목: 좌측 아이콘 + 라벨, hover 시 bg-accent
// 현재 페이지 항목: font-bold + bg-accent
```
...
```
데스크톱 (>1024px):
  상단 헤더바에 6개 그룹, hover 드롭다운

태블릿 (640~1024px):
  상단 헤더바 유지, 그룹이 넘치면 가로 스크롤
  또는 햄버거 메뉴로 전환
...
**E. i18n 키 추가** (ko.json / ja.json / en.json):
...
**F. 구현 순서**:
```
1. config/adminNavigation.ts — 위 데이터 구조 생성
2. components/nav/NavDropdown.tsx — 드롭다운 컴포넌트
3. components/nav/NavGroup.tsx — 그룹 렌더러 (역할 필터링)
4. components/nav/MobileNav.tsx — 모바일 아코디언 메뉴
5. AdminLayout.tsx — 기존 1단 navKeys 배열 → adminNavGroups로 교체
6. 역할별 필터링 훅: useFilteredNav(role) → 접근 가능 그룹만 반환
7. i18n ko.json, ja.json, en.json 키 추가
8. 기존 라우트(App.tsx) 변경 없음 — 메뉴 표시만 변경
```
