...
export default function ReceivingPage() {
  ...
  const [newDelivery, setNewDelivery] = useState({ storeId: '1', supplierId: '' });
  ...
  const handleConfirm = async () => {
    ...
    if (!confirm('Confirm this delivery?')) return;
    try {
  ...
  const handleCreate = async () => {
    try {
      await inventoryApi.createDelivery({
        storeId: Number(newDelivery.storeId),
    ...
    return (
      <div>
        ...
        <div className="flex items-center gap-3 mb-4">
          ...
          <h2 className="text-xl font-bold flex-1">Delivery #{selectedDelivery.id}</h2>
              ...
              <div className="space-y-3">
                ...
                <div className="flex gap-2">
                  <div className="flex-1">
                    <Label className="text-xs text-gray-500">Packaging ID</Label>
                    <Input
        ...
        {deliveryPending.length > 0 && (
          <div className="mb-4">
            <p className="text-sm font-medium text-yellow-700 mb-2">
              Pending sync ({deliveryPending.length})
            </p>
          ...
          {scans.map((scan) => (
            <Card key={scan.id}>
              <CardContent className="py-3 flex justify-between items-center">
                <div>
                  <span className="font-medium">Packaging #{scan.packagingId}</span>
                  {scan.lotNo && <span className="text-gray-500 text-sm ml-2">Lot: {scan.lotNo}</span>}
                </div>
        ...
        {selectedDelivery.status !== 'COMPLETED' && (scans.length > 0 || deliveryPending.length > 0) && (
          ...
          >
            {deliveryPending.length > 0
              ? `Sync ${deliveryPending.length} scans first`
              : 'Confirm Delivery'}
          </Button>
        )}
      </div>
  ...
  return (
    <div>
      ...
      <div className="space-y-2">
          ...
          <SwipeableCard key={d.id} onSwipeRight={() => openScanView(d)} rightLabel="Open">
            <Card className="cursor-pointer" onClick={() => openScanView(d)}>
              <CardContent className="py-4 flex justify-between items-center">
                <div>
                  <span className="font-bold text-lg">Delivery #{d.id}</span>
                  <span className="text-gray-500 ml-3">Supplier #{d.supplierId}</span>
                </div>
        ...
        {deliveries.length === 0 && (
          <p className="text-center text-gray-500 py-12">No deliveries</p>
        )}
      </div>
      ...
      <Dialog open={createOpen} onOpenChange={setCreateOpen}>
        <DialogContent>
          ...
          <DialogFooter>
            ...
            <Button onClick={handleCreate} className="bg-blue-800 hover:bg-blue-900">Create</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
