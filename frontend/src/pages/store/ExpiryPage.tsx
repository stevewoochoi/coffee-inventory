...
function getStatusBadge(status: ExpiryAlert['alertStatus']) {
  switch (status) {
    case 'CRITICAL':
      return <span className="px-2 py-1 text-xs font-bold rounded-full bg-red-600 text-white">D-3 CRITICAL</span>;
    case 'WARNING':
      return <span className="px-2 py-1 text-xs font-bold rounded-full bg-yellow-500 text-white">D-7 WARNING</span>;
    case 'EXPIRED':
      return <span className="px-2 py-1 text-xs font-bold rounded-full bg-gray-500 text-white">EXPIRED</span>;
    default:
      return <span className="px-2 py-1 text-xs font-bold rounded-full bg-green-500 text-white">NORMAL</span>;
  }
}
    ...
    <div>
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        <div className="bg-red-50 border border-red-200 rounded-lg p-4 text-center">
          ...
          <div className="text-sm text-red-600 font-medium">Critical (D-3)</div>
        </div>
        <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 text-center">
          <div className="text-3xl font-bold text-yellow-600">{warningCount}</div>
          <div className="text-sm text-yellow-600 font-medium">Warning (D-7)</div>
        </div>
              ...
              >
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="font-semibold text-lg">Item #{alert.itemId}</span>
                  </div>
                ...
                <div className="mt-2 flex items-center gap-6 text-sm">
                  <span>Lot: {alert.lotNo || '-'}</span>
                  <span>Exp: {alert.expDate}</span>
                  <span>Qty: {alert.qtyBaseUnit}</span>
                </div>
              </div>
            );
          })}
