      ...
      {TIMELINE_STEPS.map((step, idx) => (
        <div key={step} className="flex items-center flex-1">
          <div className="flex flex-col items-center">
            ...
            <span className={`text-xs mt-1 ${idx <= currentIdx ? 'text-[#343741] font-medium' : 'text-gray-400'}`}>
              {t(`claims.status.${step}`)}
            </span>
          </div>
          {idx < TIMELINE_STEPS.length - 1 && (
            <div className={`flex-1 h-0.5 mx-1 ${idx < currentIdx ? 'bg-[#0077cc]' : 'bg-gray-200'}`} />
          )}
        </div>
      ))}
    </div>
          ...
          <div className="grid grid-cols-2 gap-3 text-sm">
            ...
            {claim.orderPlanId && (
              <div>
                ...
                <p className="font-medium cursor-pointer text-[#69707d]" onClick={() => navigate(`/store/ordering/${claim.orderPlanId}`)}>
                  #{claim.orderPlanId}
                </p>
              </div>
            )}
